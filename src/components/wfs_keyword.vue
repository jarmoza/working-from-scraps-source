<template>
	...
	<div class="wfs_pagelevel_limits">
		...
		<!-- Navbar features keyword and project title -->
		<wfs-nav id="keyword_navbar" :title="this.cp.collection.ids_to_keywords[$route.params.keywordID]" color="rgb(35, 43, 43)">
	...
	methods: {
		...
		getBookCount: function() {
			...
			for ( let index = 0; index < this.cp.myJSONList.length; index++ ) {
				...
				if ( this.cp.currentKeyword in this.cp.myJSONList[index].book.stats.keyword_count_dict ) {
					bookCount += 1;
				}
			}
		...
		getCollectionOccurrences: function() {
			...
			return this.cp.collection.keyword_count_dict[this.cp.currentKeyword];
		},
		...
		getPageCount: function() {
			...
			for ( let index = 0; index < this.cp.myJSONList.length; index++ ) {
				...
				for ( let index2 = 0; index2 < this.cp.myJSONList[index].pages.length; index2++ ) {
					...
					if ( this.cp.myJSONList[index].pages[index2].keywords.indexOf(this.cp.currentKeyword) > -1 ){
